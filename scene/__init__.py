...
from utils.general_utils import TorchToPIL
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        self.test_camera = {}
        self.video_camera = {}
        # if (os.path.exists(os.path.join(args.source_path, "sparse")) and 
        #     os.path.exists(os.path.join(args.source_path, "images")) and
        #     any(f.endswith(('.png', '.jpg', '.jpeg')) and 
        #         os.path.splitext(f)[0][:4].isdigit() 
        #         for f in os.listdir(os.path.join(args.source_path, "images")) 
        #         if os.path.isfile(os.path.join(args.source_path, "images", f)))):
        #     print("Found UB4D-style dataset with timestamp filenames!")
        #     scene_info = sceneLoadTypeCallbacks["UB4D"](args.source_path, args.white_background, args.eval, args.extension)
        #     dataset_type = "ub4d"
        ...
        print("Loading Training Cameras: ",len(self.train_camera))
        
        self.test_camera = FourDGSdataset(scene_info.test_cameras, args, dataset_type)
        print("Loading Test Cameras",len(self.test_camera))
        
        self.video_camera = FourDGSdataset(scene_info.video_cameras, args, dataset_type)
        print("Loading Video Cameras",len(self.video_camera))
        ...
        if self.loaded_iter:
            self.gaussians.load_ply_sparse_gaussian(os.path.join(self.model_path,
                                                           "point_cloud",
                                                           "adaptive_iteration_" + str(self.loaded_iter),
                                                           "point_cloud.ply"))
    ...
    def save(self, iteration, stage):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/{}_iteration_{}".format(stage,iteration))
